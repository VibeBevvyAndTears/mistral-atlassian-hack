  ...
  return (
    <div className="flex flex-col gap-4">
      <fieldset className="flex flex-col gap-2 border-0 p-0">
        <legend className="mb-2 text-sm font-medium text-foreground">Organization</legend>
          ...
          {localMemberships.map((org) => (
            <div key={org.org_id} className="flex items-center gap-1">
              <Button
                ...
                aria-pressed={org.org_id === selectedOrgId}
                onClick={() => {
      ...
      {selectedOrg ? (
        <TeamPickerSection
          ...
          onStartCreateTeam={() => setCreatingTeam(true)}
          onCancelCreateTeam={() => {
            setCreatingTeam(false);
            setNewTeamName("");
            setCreateTeamStatus(null);
          ...
          onCreateTeam={() => void onCreateTeam(selectedOrg.org_id)}
        />
      ) : null}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This hides it from the picker. Data is not deleted and can be restored later.
            </DialogDescription>
          </DialogHeader>
...
function TeamPickerSection({
  ...
  selectedOrg: MyOrgMembership;
  navigating: boolean;
  creatingTeam: boolean;
  newTeamName: string;
  createTeamStatus: string | null;
  savingTeam: boolean;
  onSelectTeam: (orgId: string, teamId: string, role: string) => void;
  onArchiveTeam: (team: MyTeamMembership) => void;
  ...
  const hasTeams = selectedOrg.teams.length > 0;

  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm text-muted-foreground" id="onboarding-team-label">
        Team
      </p>

      {hasTeams ? (
        <ul className="flex flex-col gap-2" aria-labelledby="onboarding-team-label">
          {selectedOrg.teams.map((team) => (
            <li key={team.team_id} className="flex items-center gap-2">
              <Button
                ...
                disabled={navigating}
                onClick={() => onSelectTeam(selectedOrg.org_id, team.team_id, team.role)}
              >
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="ghost"
                disabled={savingTeam}
                onClick={onCancelCreateTeam}
              >
                Cancel
              </Button>
            </div>
          </div>
          {createTeamStatus ? <p className="text-sm text-destructive">{createTeamStatus}</p> : null}
        </div>
      ) : null}
